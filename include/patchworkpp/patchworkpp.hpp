...
/*
    ...
    这是一个C++程序，用于将点云分成不同的区域。以下是该文件的一些函数的注释：

- temporal_ground_revert：对于每个候选点，计算其概率，以确定是否将其视为地面点。
- extract_piecewiseground：从点云中提取地面点。
- set_polygons：设置多边形的点，用于可视化。
- set_ground_likelihood_estimation_status：根据地面点的高度和平坦度，设置多边形的颜色。
- calc_mean_stdev：计算给定向量的平均值和标准差。
- xy2theta：将点的x和y坐标转换为极角。
- xy2radius：将点的x和y坐标转换为极径。
- pc2czm：将点云分成不同的区域。
*/
